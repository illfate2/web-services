...
export const EditMuseumItem = () => {
    ...
    const [updateItem] = useMutationWithAuthErrHandling(UPDATE_ITEM_QUERY, {
        onCompleted: data => {
    ...
    const {loading: funds_loading, data: funds_data} = useQueryWithAuthErrHandling(
        GET_FUNDS_QUERY
    );
    const {loading: sets_loading, data: sets_data} = useQueryWithAuthErrHandling(GET_SETS_QUERY);

    const {loading: item_loading, data: item_data} = useQueryWithAuthErrHandling(GET_ITEM_QUERY, {
        variables: {
            id: id
        },
        onCompleted: () => {
