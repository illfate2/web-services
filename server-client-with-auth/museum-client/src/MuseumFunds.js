...
function CreateFundForm() {
  let input;
  const [addFund, { data }] = useMutationWithAuthErrHandling(CREATE_FUND_QUERY);
  return (
    <div>
      <form
        onSubmit={e => {
...
const MuseumFunds = () => {
  let { loading, error, data } = useQueryWithAuthErrHandling(GET_FUNDS_QUERY);

  const [deleteFund, { deleteData }] = useMutationWithAuthErrHandling(
    DELETE_FUND_QUERY
  );
