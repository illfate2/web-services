...
func (r *mutationResolver) CreateMuseumSet(ctx context.Context, input model.MuseumSetInput) (*model.MuseumSet, error) {
	set, err := r.service.CreateMuseumSet(entities.MuseumSet{
		Name:      input.Name,
		CreatedBy: ctx.Value("userID").(int),
	})
	if err != nil {
		return nil, err
	}
...
func (r *Resolver) SignupUser(ctx context.Context, input *model.Signup) (*model.AuthResponse, error) {
	user, err := r.service.CreateUser(entities.User{
		Email:    input.Email,
	...
	accessToken, err := r.jwtService.GenerateAccessToken(user.ID)
	if err != nil {
		return nil, err
	}
	refreshToken, err := r.jwtService.GenerateRefreshToken(user.ID)
	if err != nil {
		return nil, err
	}
